$(function() {
    ...
    $("select[name='ResNotice[client_id]']").change(function() {
        ...
        if (!isNaN(parseInt(clientID))) {
            $.get(actionUrl, { clientID: clientID }, function(data) {
                var el = $("select[name='ResNotice[fire_id]']");
                el.html(data);
                flashSelectMenu(el);
                $("select[name='ResNotice[notice_id]']").html('');
    ...
    $("select[name='ResNotice[fire_id]']").change(function() {
        var clientSelect = $("select[name='ResNotice[client_id]']")[0];
        var actionUrl = this.getAttribute('data-url');
        ...
        if (!isNaN(parseInt(fireID))) {
            $.get(actionUrl, { clientID: clientID, fireID: fireID }, function(data) {
                var el = $("select[name='ResNotice[notice_id]']");
                el.html(data);
                flashSelectMenu(el);
            }, 'html').error(function(jqXHR) {
                console.log(jqXHR);
    ...
    $("select[name='ResNotice[notice_id]']").change(function() {
