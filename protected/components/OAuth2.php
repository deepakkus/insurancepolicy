...
/**
 * The default duration in seconds of the access token lifetime. 1 hr (3600 seconds)
 */
...
abstract class OAuth2 {
  ...
  /**
   * Get YOauth2User representing the oauth2 user of corresponding client_id.
   *
   * @param $client_id
   *   Client identifier to be check with.
   *
   * @return YOauth2User
   */
  abstract protected function getOauth2User($client_id);
  ...
  public function verifyAccessToken($scope = NULL, $exit_not_present = TRUE, $exit_invalid = TRUE, $exit_expired = TRUE, $exit_scope = TRUE, $realm = NULL) {
    ...
    // Check token expiration (Added "isset" check in there for clients that don't set expiration and leave it a NULL backwards compatibility)
    ...
    if ($scope && (!isset($token["scope"]) || !$token["scope"] || !$this->checkAccessTokenScope($scope, $token["scope"])))
  ...
  /**
   * Check if availible scope contains the the required scope
   *
   * @param $required_scope
   *   Required scope to be check with.
   * @param $available_scope
   ...
   * @return boolean
   */
  private function checkAccessTokenScope($required_scope, $available_scope) {
      // The required scope should match or be a subset of the available scope
      if (!is_array($required_scope))
          $required_scope = explode(" ", $required_scope);
      ...
      return (count(array_intersect($required_scope, $available_scope)) >= 1);
  }
  ...
  public function grantAccessToken() {
    ...
    switch ($input["grant_type"]) {
      case OAUTH2_GRANT_TYPE_AUTH_CODE:
        if (!$input["code"] || !$input["redirect_uri"])
          $this->errorJsonResponse(OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'Missing parameters. "code" and "redirect_uri" required. '. var_export($input));

        $stored = $this->getAuthCode($input["code"]);

        // Scope is required for any user other than ios
        if ($client[0] !== 'iOSfireshield0001' && !$input["scope"])
            $this->errorJsonResponse(OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'Missing parameter. "scope" required.');

        // Ensure that the input uri starts with the stored uri and that the client_id matches
        if ($stored === NULL || (strcasecmp(substr($input["redirect_uri"], 0, strlen($stored["redirect_uri"])), $stored["redirect_uri"]) !== 0) || $client[0] != $stored["client_id"])
            $this->errorJsonResponse(OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'Auth Code Param Mismatch');
    ...
    if (!$input["scope"] && $client[0] === 'iOSfireshield0001')
        $input["scope"] = 'fireshield';

    // Check scope, if provided
    if (!is_array($stored) || !isset($stored["scope"]) || !$this->checkScope($input["scope"], $stored["scope"]))
  ...
  public function getAuthorizeParams() {
	...
	if(filter_has_var(INPUT_GET, 'client_id'))
	{
		$input = filter_input_array(INPUT_GET, $filters);
	}
	else if(filter_has_var(INPUT_POST, 'client_id'))
	{
		$input = filter_input_array(INPUT_POST, $filters);
	}
	else //if (!$input["client_id"]) {
	{
		...
		//	$this->errorDoRedirectUriCallback($input["redirect_uri"], OAUTH2_ERROR_INVALID_CLIENT, "Missing client_id GET or POST variable", NULL, $input["state"]);

		$this->errorJsonResponse(OAUTH2_HTTP_FOUND, OAUTH2_ERROR_INVALID_REQUEST, "Missing client_id GET or POST variable"); // We don't have a good URI to use
    }

    // Scope is required for any user other than ios
    if ($input["client_id"] !== 'iOSfireshield0001' && !$input['scope'])
        $this->errorJsonResponse(OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'Missing parameter "scope" required.');

    // Get an object with variables assigned to this oa2 user
    $authUser = $this->getOauth2User($input["client_id"]);

    // getOauth2User will return false if client was not found
    // If this is the case, having $input["redirect_uri"] exist is not reliable, do a direct errorJsonResponse
    if ($authUser === FALSE)
      $this->errorJsonResponse(OAUTH2_HTTP_FOUND, OAUTH2_ERROR_INVALID_CLIENT, "OAuth2 client could not be found");

    $redirect_uri = $authUser->redirectUri;

    // User has been deactivated
    if ($authUser->active !== true)
      $this->errorDoRedirectUriCallback($redirect_uri, 'deactivated user', NULL, NULL, $input["state"]);

    // At least one of: existing redirect URI or input redirect URI must be specified
    if (!$redirect_uri && !$input["redirect_uri"])
      $this->errorJsonResponse(OAUTH2_HTTP_FOUND, OAUTH2_ERROR_INVALID_REQUEST, "Missing redirect_uri GET or POST variable");
    ...
    else if ($redirect_uri) { // They did not provide a uri from input, so use the stored one
      $input["redirect_uri"] = $redirect_uri;
    }

    // type and client_id are required
    if (!$input["response_type"])
      $this->errorDoRedirectUriCallback($input["redirect_uri"], OAUTH2_ERROR_INVALID_REQUEST, "Missing response_type GET variable", NULL, $input["state"]);
    ...
    if ($input["scope"] && !$this->checkScope($input["scope"], $authUser->scope))
  ...
  protected function createAccessToken($client_id, $scope = NULL) {
	...
	$this->setAccessToken($token["access_token"], $client_id, $token['expires'], $scope);

    // Issue a refresh token also, if we support them
    if (in_array(OAUTH2_GRANT_TYPE_REFRESH_TOKEN, $this->getSupportedGrantTypes())) {
      $token["refresh_token"] = $this->genAccessToken();
      $token["refresh_expires"] = time() + $this->getVariable('refresh_token_lifetime', OAUTH2_DEFAULT_REFRESH_TOKEN_LIFETIME);
      $this->setRefreshToken($token["refresh_token"], $client_id, $token["refresh_expires"], $scope);
