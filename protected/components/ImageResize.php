...
class ImageResize
{
    ...
    protected function load($filename)
    {
        ...
        $this->exif_data = @exif_read_data($filename);
    ...
    public function save($filename, $image_type = null, $quality = null, $permissions = null)
    {
        ...
        // Flip the image if the metadata indicates it's needed
        // Do this because the PHP GD Library functions strip the EXIF metadata
        $orientation = isset($this->exif_data['Orientation']) ? $this->exif_data['Orientation'] : null;

        if ($orientation !== null) {
            switch ($orientation) {
                case 3:
                    $dest_image = imagerotate($dest_image, 180, 0);
                    break;

                case 6:
                    $dest_image = imagerotate($dest_image, -90, 0);
                    break;
    ...
    public function rotateIfNeeded()
    {
        ...
        if ($orientation !== null) {
            switch ($orientation) {
                case 3:
                    $this->source_image = imagerotate($this->source_image, 180, 0);
                    break;

                case 6:
                    $this->source_image = imagerotate($this->source_image, -90, 0);
                    break;
