...
$threats = ResPerimeters::getThreatsForFireID($model->fire_id);
...
<div class="row-fluid">
    <div class="span6">
        ...
        <h2>Choose a Threat Perimeter</h2>

        <p class="lead">
            <u>Upload file or choose from existing threats</u>
        </p>
        ...
        <div class="section">
            ...
            if ($threats)
            {
                echo $form->radioButtonListRow($model, 'threatIDToCopy', CHtml::listData($threats, 'threat_location_id', function($data) {
...
<script type="text/javascript">
    ...
    var threats = <?php echo json_encode(CHtml::listData($threats, 'threat_location_id', 'id')); ?>;
    ...
    $("input[type=radio][name='<?php echo CHtml::activeName($model, 'threatIDToCopy'); ?>']").change(function() {
        map.fire("threat", {
            perimeterID: threats[this.value]
        });
    });

    // Does this web browser support HTML file APIs?
    // If so, display file uploaded threat on map
    if (window.File && window.FileReader && window.FileList && window.Blob) {
        $("#<?php echo CHtml::activeId($model, 'kmlFileUpload'); ?>").on('change', function(event) {
            // Remove radio button checks
            $("input[name='<?php echo CHtml::activeName($model, 'threatIDToCopy'); ?>']").removeAttr("checked");
            // Display file on map if possible
                ...
                reader.onload = (function(file) {
                    return function() {
                        if (file.type === 'application/vnd.google-earth.kml+xml') {
                            ...
                            threatLayer = omnivore.kml.parse(this.result).eachLayer(function(layer) {
                                layer.setStyle(Threat.threatStyle(layer.feature));
                            }).addTo(map);
                             map.fitBounds(threatLayer.getBounds());
                        } else {
                            // Only KMLs will be displayed on map
    ...
    $.getJSON('<?php echo $this->createUrl('resPerimeters/getPerimeterGeoJson', array('perimeterID' => $model->id)); ?>', function(featureCollection) {
    ...
    // If this model is already associated with a threat, load it

    if ($model->threat_location_id)
    {
        echo 'map.fire("threat", { perimeterID: ' . $model->id . ' });';
    }
