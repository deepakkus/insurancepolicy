...
var EvacZone = function () {
    this.init();
};

EvacZone.prototype.init = function () {
    this.id = null;
    this.noticeId = null;
    this.geog = null;
    this.notes = null;
...
// Leaflet Evac Zone plugin

L.Control.EvacControl = L.Control.extend({

    initialize: function (evacZones, editableLayers, options) {
        L.Util.setOptions(this, options);
        this.evacZones = evacZones;
    ...
    onAdd: function (map) {
        ...
        var container = L.DomUtil.create('div', 'leaflet-evactoolbar');
        ...
        var layerContainer = this.addEvacLayerContainer();
    ...
    addUpdateLayers: function () {
        for (var i = 0; i < this.evacZones.length; i++) {
            var zone = this.evacZones[i];
            var geojson = JSON.parse(zone.geog);
            ...
            // Adding to widget
            polygon.notes = zone.notes;
            polygon.bindLabel('Evac Zone: ' + zone.notes);
            this.addEvacLayerToTable(polygon);
        }
    },
    ...
    setupLeafletDraw: function () {
        ...
        L.drawLocal.draw.toolbar.buttons.polygon = 'Draw a new evac zone!';
        L.drawLocal.draw.handlers.polygon.tooltip.start = 'Click to start drawing a evac zone.';
        L.drawLocal.draw.handlers.polygon.tooltip.edit = 'Click to continue drawing this evac zone.';
        L.drawLocal.draw.handlers.polygon.tooltip.end = 'Click first point to close this evac zone.';
        L.drawLocal.draw.handlers.polygon.tooltip.end = 'Click first point to close this evac zone.';
    ...
    setupLeafletDrawListeners: function () {
        ...
        this.map.on(L.Draw.Event.CREATED, function (e) {
            ...
            this.addEvacLayerToTable(e.layer);
        }.bind(this));

        this.map.on(L.Draw.Event.DELETED, function (e) {
            e.layers.getLayers().forEach(function (layer) {
                this.removeEvacLayerFromTable(e.layers.getLayerId(layer));
            }.bind(this));
        }.bind(this));
    ...
    // Creating DOM to hold the evac zones
    addEvacLayerContainer: function () {
        var container = L.DomUtil.create('div', 'leaflet-evactoolbar-section');
        var evacContainer = L.DomUtil.create('div', 'leaflet-evactoolbar-toolbar leaflet-bar', container);
        var evacDiv = L.DomUtil.create('div', 'leaflet-wdstoolbar-evaclayers', evacContainer);

        this.evacLayersTable = document.createElement('table');
        this.evacLayersTable.id = 'evac-layers-table';
        this.evacLayersTable.innerHTML = '<tr>' +
            '<th>Layer</th><th>Notes</th>' +
            '</tr>';

        evacDiv.appendChild(this.evacLayersTable);

        return container;
    },

    // Creating DOM to submit the evac zones
    addSubmitButtonContainer: function () {
        var container = L.DomUtil.create('div', 'leaflet-evactoolbar-section');
        var sumbitContainer = L.DomUtil.create('div', 'leaflet-evactoolbar-toolbar leaflet-bar', container);
        ...
        L.DomEvent.on(submitButton, 'click', this.submitEvacZones, this);
    ...
    addUploadContainer: function () {
        var container = L.DomUtil.create('div', 'leaflet-evactoolbar-section');
        var uploadContainer = L.DomUtil.create('div', 'leaflet-evactoolbar-toolbar leaflet-bar', container);
                    ...
                    return function () {
                        if (file.type === 'application/vnd.google-earth.kml+xml') {
                            ...
                            omnivore.kml.parse(contents).eachLayer(function (layer) {
                                ...
                                _this.addEvacLayerToTable(polygon);
                                // Creating bounds
    ...
    // Adding evac layer to table by layer ID
    addEvacLayerToTable: function (layer) {
        ...
        tdElement1.setAttribute('class', 'evac-results');
        ...
        this.evacLayersTable.appendChild(trElement);
    ...
    // Removing evac layer from table by layer ID
    removeEvacLayerFromTable: function (layerID) {
        var trElement = this.evacLayersTable.querySelector('tr[data-layer-id="' + layerID + '"]');
        this.evacLayersTable.removeChild(trElement);
    },

    // Binding even listeners to table tr element
    bindEventListeners: function (trElement, notesElement, layer) {
        // Darken evac zone and trElement on trElement mouseover
        ...
        // Light evac zone and trElement on trElement mouseover
        ...
        notesElement.addEventListener('input', function (e) {
            layer.bindLabel('Evac Zone: ' + e.target.value);
        });
    },
    ...
    // Sumbitting evac zones
    submitEvacZones: function (e) {

        var evacZones = [];
        var trElements = this.evacLayersTable.querySelectorAll('tr');
        var noticeSelect = document.querySelector('select[name="ResNotice[notice_id]"]');
        ...
        // Creating javascript evac zones to pass to Admin
        for (var i = 0; i < trElements.length; i++) {
            ...
            if (layerID) {
                ...
                var evacZone = new EvacZone();
                evacZone.geog = layer.toGeoJSON();
                evacZone.notes = notes;

                evacZones.push(evacZone);
            }
        }
        ...
        $.post(window.location.href, {
            noticeID: noticeID,
            evacZones: JSON.stringify(evacZones)
        }, function (data) {
            if (data === true) {
                this.actionLoading('remove');
                window.location.href = '/index.php?r=resEvacZone/admin';
            } else {
...
L.control.evacControl = function (options) {
    return new L.Control.EvacControl(options);
};
